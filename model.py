...
class RiskEvalModel:
    ...
    def train_run(self):
        ...
        param_dist = {"clf__C": np.logspace(-5, 7, 50)}
    ...
    def plot(self, roc, target_recall):
        ...
        precision, recall, risk_tolerances = precision_recall_curve(
        ...
        axes["confusion"].set_title(
            f"Confusion Matrix (risk_tolerance={risk_tolerance:.2f})"
        )
    ...
    def evaluate(self):
        ...
        tn, fp, fn, tp = confusion_matrix(
            self.y_test, self.y_pred_thr
        ).ravel()  # Gives the FP,FN,TP,TN of the model
...
class FailureClassificationModel:
    def __init__(self, df: pd.DataFrame, target, classifications, ignore=None):
        ...
        self.classifications = (
            classifications  # List of class labels e.g. ['TWF','HDF','PWF','OSF']
        )
    ...
    def dataset(self):
        ...
        drop_cols = [
            c
            for c in (self.ignore + [self.target, "_failure_class"])
        ...
        print(
            f"Classification dataset: {len(self.X)} failure rows, class distribution:"
        )
        print(
            self.y.value_counts()
            .sort_index()
            .rename(
                index={
                    i: self.classifications[i] for i in range(len(self.classifications))
                }
            )
        )
    ...
    def train_run(self):
        ...
        param_dist = {"clf__C": np.logspace(-2, 2, 20)}
    ...
    def plot(self):
        ...
        class_labels = [
            self.classifications[i] if i < len(self.classifications) else f"Class {i}"
            for i in range(n_classes)
        ]
        ...
        axes["confusion"].bar(
            x, rec_vals, width=bar_w, label="Recall", color=bar_palette[1], alpha=0.85
        )
        axes["confusion"].bar(
            x + bar_w,
            f1_vals,
...
class FailurePredictionSystem:
        ...
        for w_s in warning_range:
            for d_s in diagnosis_range:
                ...
                if first_warning_step:
                    score = 1.0 / (
                        abs(target_step_for_warning - first_warning_step) + 1
                    )
                else:
                    score = 0
                ...
                if score > best_score:
                    best_score = score
                    best_params = {
                        "warning_sensitivity": w_s,
                        "diagnosis_sensitivity": d_s,
                    }
    ...
    def evaluate_test_set(self, test_df):
        ...
        for i in range(len(test_df)):
            ...
            print(
                f"{i+1:<5} | {result['risk_prob']:<8.4f} | {result['alert_level']:<10} | {result['failure_type']}"
            )
...
if __name__ == "__main__":
    ...
    best_config = FinalSystem.calibrate_sensitivities(
        test_data, target_step_for_warning=4
    )

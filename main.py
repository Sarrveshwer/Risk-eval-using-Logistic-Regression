...
class FailureClassificationModel:
    def __init__(self, filename, target, classifications, ignore=None):
        filename_modified, ext = os.path.splitext(filename)
        self.ext = ext
        self.filename = filename_modified
        self.target = target
        self.classifications = classifications # List of class labels (e.g., [0, 1, 2, 3])
        self.ignore = ignore if ignore is None else ignore
    ...
    def dataset(self):
        df = pd.read_csv(self.filename + self.ext)
        
        # Softmax requires the target to contain all class indices (0 to N)
        y = df[self.target].astype(int)
        X = df.drop(columns=self.ignore + [self.target])
    ...
    def plot(self):
        ...
        sns.heatmap(
            ...
            linewidths=0.5,
            linecolor="white",
            xticklabels=self.classifications[:n_classes],
            yticklabels=self.classifications[:n_classes],
            alpha = 0.8,
            cbar=False,
            ax=axes["confusion"],
        )

        axes["confusion"].set_title("Multiclass Confusion Matrix")
        axes["confusion"].set_xlabel("Predicted Label")
        axes["confusion"].set_ylabel("Actual Label")
...
class FailurePredictionSystem:
    ...
    def LoadModels(self):
        ...
        if os.path.exists(softmax_path):
            ...
            clf = FailureClassificationModel(
                self.filename,
                self.target,
                self.classifications, # Fixed order to match __init__
                self.ignore
            )
    ...
    def predict(self, X: pd.DataFrame):
        ...
        for col in dynamic_features:
            # Rolling mean doesn't need the Safety net
            current_data_df[f'{col}_Rolling_Mean'] = history_dataframe[col].mean() 
            
            if len(self.history) > 1: # acts a safety net so that it doesn't throw a ZeroError
